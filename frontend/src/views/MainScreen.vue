<template>
  <div class="battle-screen">
    ...
    <div class="battle-content">
      ...
      <aside class="feed-panel battle-panel" aria-label="战斗日志与留言板">
        ...
        <MessageBoardPanel
          ...
          :active="mainFeedTab === 'chat'"
        />
        <LeaderboardPanel
          v-show="mainFeedTab === 'leaderboard'"
          :active="mainFeedTab === 'leaderboard'"
        />
      </aside>
    </div>
    ...
    <ShopModal
      ...
      :shop-confirming-slot="shopConfirmingSlot"
      :squad-max-level="squadMaxLevel"
      @close="closeShopModal"
      @select-slot="shopConfirmingSlot = $event"
      @confirm-buy="confirmShopBuy"
    ...
    <PlayerStatsModal
      ...
      :exploration-steps-display="explorationStepsDisplay"
      :stats-scale-n="statsScaleN"
      :player-stats-win-rate-pie="playerStatsWinRatePie"
      :player-stats-win-rate-summary="playerStatsWinRateSummary"
      :player-stats-battle-timeline="playerStatsBattleTimeline"
      :player-stats-timeline-chart-model="playerStatsTimelineChartModel"
      :player-stats-damage-share-pie="playerStatsDamageSharePie"
      :player-stats-damage-squad-total="playerStatsDamageSquadTotal"
      :player-stats-per-hero-damage-pies="playerStatsPerHeroDamagePies"
      :player-stats-injury-share-pie="playerStatsInjurySharePie"
      :player-stats-injury-squad-total="playerStatsInjurySquadTotal"
      :player-stats-per-hero-injury-pies="playerStatsPerHeroInjuryPies"
      @close="showPlayerStatsModal = false"
      @set-scale="setStatsDisplayScale"
      @reset="confirmResetPlayerStats"
    />
    ...
    <ItemDetailModal
      v-if="selectedItem || equipReplacePending?.mode === 'equip_confirm'"
      :mode="itemDetailMode"
      :selected-item="selectedItem"
      :equip-item="equipReplacePending?.item ?? null"
      :equip-hero="equipReplacePending?.hero ?? null"
      :target-slot="equipReplacePending?.targetSlot ?? ''"
      :replace-current="replaceCompareCurrent"
      :squad="squad"
      :inventory-item-ids="inventoryItemIds"
      :ring-choice="equipReplacePending?.mode === 'ring_choice' ? equipReplacePending.hero : null"
      @close="closeItemDetail"
      @confirm-replace="confirmEquipReplace(equipReplacePending.item, equipReplacePending.hero, equipReplacePending.targetSlot); equipReplacePending = null; selectedItem = null"
      @cancel-replace="equipReplacePending = null"
      @confirm-equip="confirmEquipReplace(equipReplacePending.item, equipReplacePending.hero, equipReplacePending.targetSlot); equipReplacePending = null"
      @ring-choice="onRingChoice"
      @equip-to-hero="handleEquipToHero(selectedItem, $event)"
      @sell="confirmSellItem(selectedItem); sellConfirmingItem = null"
    />
    ...
    <ItemDetailModal
      v-if="selectedEquippedItem"
      mode="detail"
      :selected-item="selectedEquippedItem.item"
      show-unequip
      :inventory-item-ids="[]"
      @close="selectedEquippedItem = null; equippedUnequipConfirming = false"
...
import PlayerStatsModal from '../components/panels/PlayerStatsModal.vue'
import MessageBoardPanel from '../components/panels/MessageBoardPanel.vue'
import LeaderboardPanel from '../components/panels/LeaderboardPanel.vue'
...
import { buyFromShopOnServer } from '../game/shop.js'
import {
...
/** ItemDetailModal render mode derived from equipReplacePending / selection. */
const itemDetailMode = computed(() => {
  const m = equipReplacePending.value?.mode
  if (m === 'replace_confirm') return 'replace_confirm'
  if (m === 'equip_confirm') return 'equip_confirm'
  return 'detail'
})

/** Inventory ids for ItemDetailModal's sell-price row. */
const inventoryItemIds = computed(() => getInventory().map((i) => i.id))

/** Close item detail and clear its orchestration state. */
function closeItemDetail() {
  selectedItem.value = null
  sellConfirmingItem.value = null
  equipReplacePending.value = null
}

/** Ring-choice selection in item detail (equipReplacePending mode ring_choice). */
function onRingChoice(slot) {
  const p = equipReplacePending.value
  if (!p) return
  confirmEquipReplace(p.item, p.hero, slot)
  equipReplacePending.value = null
  selectedItem.value = null
}
...
function uninstallCombatVisibilityRecovery() {
  if (typeof document === 'undefined' || !onCombatVisibilityChange) return
  document.removeEventListener('visibilitychange', onCombatVisibilityChange)
  onCombatVisibilityChange = null
}
