...
- **PWS auto-excludes shielded allies**: The engine **automatically** filters out allies who already have an active shield when picking targets for power-word-shield. You do NOT need to add a special "target-no-shield" condition for ally targeting; the engine handles it. When the player says "给无盾的队友套盾" or "自己有盾就给队友套", just use **lowest-hp-ally** (or other ally rule) and the engine will skip shielded allies. If **all** allies have shields, the engine falls back to the full pool (refresh lowest).
