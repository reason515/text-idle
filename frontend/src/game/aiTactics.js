  ...
  "conditions": [
    {
      ...
      "whenAll": [{ "when": "<condition type>", "value": "<optional threshold>" }]
    }
  ],
...
- **Priest: "only heal when someone below X%" — NO ungated lowest-hp-ally fallback (CRITICAL)**:
  - If the player says **only** things like "队伍里有人（含自己）HP低于60%时对其快速治疗" / "存在低于60%的英雄时施放快速治疗" and does **not** ask to heal "otherwise" / "still heal the lowest current HP" when everyone is above the threshold, output **flash-heal** \`targetRules\` as **a single step only**: \`[{ "rule": "lowest-hp-ally", "when": "ally-hp-below", "value": 0.6 }]\`.
  - **NEVER** append a second step \`"lowest-hp-ally"\` (plain, no per-step gate) after that triage step. Plain \`lowest-hp-ally\` picks by **absolute current HP** among all allies when the first step's pool is empty — so when **everyone** is above 60%, the engine falls through and may heal the priest who has the smallest HP **number** (e.g. 19/23) even at high **percentage**. That contradicts "only heal when someone is below 60%".
  - Only add **additional** chain steps after triage when the player explicitly describes other branches (e.g. self-if-enemy-targeting, tank when low, or explicit "否则对血量最少者治疗").
...
- **PWS auto-excludes shielded allies**: The engine **automatically** filters out allies who already have an active shield when picking targets for power-word-shield. You do NOT need to add a special "target-no-shield" condition for ally targeting; the engine handles it. When the player says "给无盾的队友套盾" or "自己有盾就给队友套", just use **lowest-hp-ally** (or other ally rule) and the engine will skip shielded allies. If **all** allies have shields, **power-word-shield** finds **no** target (skill skipped; next priority e.g. basic attack) — no automatic refresh via target pick.
...
WRONG 6b (Priest) - fabricating "only when targeted" for healthy-party shield + redundant gates on step 2:
Player: "队伍中有人HP低于60%时优先治疗；全员高于60%且无盾时给自己套盾，有盾则给无盾队友套盾"
WRONG: step1 \`{ "rule": "self-if-enemy-targeting", ... }\` or step2 \`{ "rule": "lowest-hp-ally", "whenAll": [{ "when": "self-hp-above", "value": 0.6 }] }\`
CORRECT: { "skillId": "power-word-shield", "targetRules": [{ "rule": "self", "whenAll": [{ "when": "self-hp-above", "value": 0.6 }, { "when": "self-no-shield" }] }, "lowest-hp-ally"] }
Reason: (1) **self-if-enemy-targeting** is wrong; use plain **self** with gates. (2) Step 2 does NOT need **self-hp-above** because: if step 1 fails due to **self-no-shield** failing (self already has shield), it implies self-hp-above already passed; if step 1 fails due to **self-hp-above** failing (self HP low), flash-heal (higher priority) handles it. (3) **lowest-hp-ally** on step 2 is fine — the engine **auto-excludes** allies who already have an active shield when targeting power-word-shield, so it naturally picks the **lowest-HP unshielded** ally. No extra "no-shield" condition needed on the step.

WRONG 13 (Priest) - power-word-shield uses **self-hp-below** for "whole party above 60% then shield" (inverted):
Player: "全队高于60%时给自己套盾；自己有盾时给无盾队友套盾" or "所有英雄HP都高于60%且自己身上无真言术盾时给自己套盾"
WRONG: step1 \`{ "rule": "self", "whenAll": [{ "when": "self-hp-below", "value": 0.6 }, { "when": "self-no-shield" }] }\` then step2 \`lowest-hp-ally\` with **self-no-shield** again
CORRECT: step1 \`{ "rule": "self", "whenAll": [{ "when": "self-hp-above", "value": 0.6 }, { "when": "self-no-shield" }] }\`, step2 \`{ "rule": "lowest-hp-ally", "whenAll": [{ "when": "self-hp-above", "value": 0.6 }] }\` — **self-hp-above** means the priest is in the "healthy" band; do **not** reuse **self-no-shield** on step2 (step1 already fails when self has shield, so step2 spreads to allies).
Reason: **Below** 60% is for **healing** (flash-heal triage). **Above** 60% + shield rules belong together for healthy-party shield rotation. Validators may auto-correct inverted gates when this buggy pair is detected.

WRONG 12 (Priest) - flash-heal triage + plain **lowest-hp-ally** fallback when player asked **only** for heal-when-below-threshold:
Player: "队伍中存在HP低于60%的英雄（含自己）时对其施放快速治疗；全队高于60%时按另一套规则套盾…"
WRONG: { "skillId": "flash-heal", "targetRules": [{ "rule": "lowest-hp-ally", "when": "ally-hp-below", "value": 0.6 }, "lowest-hp-ally"] }
CORRECT: { "skillId": "flash-heal", "targetRules": [{ "rule": "lowest-hp-ally", "when": "ally-hp-below", "value": 0.6 }] }
Reason: A plain second step \`lowest-hp-ally\` runs when **no one** is <=60% and picks the ally with **lowest current HP (absolute)**, which can be the priest at 19/23 (~83%). That is **not** "only heal when someone is below 60%". Omit the second step so the skill **skips** when the triage pool is empty. Only add more steps if the player described other heal branches.
...
WRONG 11 (Mage) - fireball only **target-hp-below** for a **middle band** while **basic attack** is gated to very low HP (overlap: below 5% also matches below 50%):
Player: "打HP最低敌人；低于5%用普通攻击；5%-50%用火球；50%以上用寒冰箭"
WRONG: { "skillPriority": ["frostbolt", "fireball"], "targetRule": "lowest-hp", "conditions": [{ "skillId": "frostbolt", "targetRule": "lowest-hp", "when": "target-hp-above", "value": 0.5 }, { "skillId": "fireball", "targetRule": "lowest-hp", "when": "target-hp-below", "value": 0.5 }, { "skillId": "basic-attack", "targetRule": "lowest-hp", "when": "target-hp-below", "value": 0.05 }] }
CORRECT: { "skillPriority": ["frostbolt", "fireball"], "targetRule": "lowest-hp", "conditions": [{ "skillId": "frostbolt", "targetRule": "lowest-hp", "when": "target-hp-above", "value": 0.5 }, { "skillId": "fireball", "targetRule": "lowest-hp", "whenAll": [{ "when": "target-hp-above", "value": 0.05 }, { "when": "target-hp-below", "value": 0.5 }] }, { "skillId": "basic-attack", "targetRule": "lowest-hp", "when": "target-hp-below", "value": 0.05 }] }
Reason: **target-hp-below 0.5** is true at 3% HP, so **fireball** would be chosen before **basic attack** unless **fireball** uses **skill-level whenAll** so it requires **target-hp-above** the low band **and** **target-hp-below** the high band (AND). **whenAll** is allowed on the **skill condition object** (same semantics as targetRules step whenAll).
...
/**
 * Mage: player describes non-overlapping HP bands on the same target (e.g. lowest HP) with frostbolt / fireball / basic attack.
 * Requires skill-level whenAll on fireball (between low and high) so sub-5% does not match fireball's "below 50%" branch.
 * @param {string|undefined} userInput
 * @param {string[]} priority
 * @param {Object[]} conditions
 * @param {string[]} skillIds
 * @param {string[]} warnings
 * @returns {{ targetRule: string }|null}
 */
function supplementMageHpBandTactics(userInput, priority, conditions, skillIds, warnings) {
  if (!userInput || typeof userInput !== 'string') return null
  if (!skillIds.includes('frostbolt') || !skillIds.includes('fireball')) return null

  const u = userInput
  const compact = u.replace(/\s/g, '')
  if (!/(最低|血量最低|HP最低)/.test(u)) return null
  if (!/寒冰箭/.test(u) || !/火球/.test(u) || !/(普通攻击|普攻)/.test(u)) return null

  const threeBand =
    /低于.{0,12}5\s*%/.test(u) &&
    (/5\s*%[-~至到]50\s*%|5.*%-.*50.*%/.test(compact) || (/之间/.test(u) && /5/.test(u) && /50/.test(u))) &&
    (/50\s*%.{0,8}以上|以上.{0,8}50|高于\s*50/.test(u) || /50.*寒冰/.test(compact))

  if (!threeBand) return null

  let low = 0.05
  let high = 0.5
  const rangeM = u.match(/(\d+)\s*%?\s*[-~至到]\s*(\d+)\s*%/)
  if (rangeM) {
    low = Number(rangeM[1]) / 100
    high = Number(rangeM[2]) / 100
  }

  warnings.push(
    '已补全法师多段血量战术：优先级 寒冰箭>火球术；火球术使用技能级 whenAll（目标血量高于下阈且不高于上阈），避免低血时误用火球。',
  )

  priority.length = 0
  priority.push('frostbolt', 'fireball')

  const kept = conditions.filter((c) => !['frostbolt', 'fireball', 'basic-attack'].includes(c.skillId))
  conditions.length = 0
  conditions.push(
    {
      skillId: 'frostbolt',
    ...
    },
    {
      ...
      targetRule: 'lowest-hp',
      whenAll: [
        { when: 'target-hp-above', value: low },
        { when: 'target-hp-below', value: high },
      ],
    },
    {
 ...
 * lowest **absolute** current HP among all allies (can be the priest at e.g. 19/23), contradicting "only heal when someone is below X%".
 * Strip that ungated fallback unless user text explicitly asks for an "otherwise still heal lowest" behavior.
 * @param {Object[]} conditions
 * @param {string|undefined} userInput
 * @param {string[]} warnings
 */
 ...
 */
/**
 * Model often outputs power-word-shield step1 as self + self-hp-below + self-no-shield when the player asked for
 * "whole party above 60% then shield self / else shield ally" (should be self-hp-above on step1).
 * Step2 may wrongly repeat self-no-shield; the second branch runs when step1 fails (typically because self already has shield).
 * @param {Object[]} conditions
 * @param {string[]} warnings
 */
function fixPriestPwsHealthyPartyShieldMistarget(conditions, warnings) {
  const pw = conditions.find((c) => c.skillId === 'power-word-shield')
  if (!pw?.targetRules || pw.targetRules.length < 2) return

  const s0 = pw.targetRules[0]
  const s1 = pw.targetRules[1]
  if (typeof s0 !== 'object' || s0 === null || s0.rule !== 'self') return
  if (!Array.isArray(s0.whenAll) || s0.whenAll.length === 0) return

  const hasBelow = s0.whenAll.some((w) => w?.when === 'self-hp-below')
  const hasNoShieldS0 = s0.whenAll.some((w) => w?.when === 'self-no-shield')
  if (!hasBelow || !hasNoShieldS0) return

  const secondIsLowest =
    s1 === 'lowest-hp-ally' ||
    (typeof s1 === 'object' && s1 !== null && s1.rule === 'lowest-hp-ally')
  if (!secondIsLowest) return

  const s1Obj = typeof s1 === 'object' && s1 !== null ? s1 : null
  const s1HasAbove =
    s1Obj &&
    ((Array.isArray(s1Obj.whenAll) && s1Obj.whenAll.some((w) => w?.when === 'self-hp-above')) ||
      s1Obj.when === 'self-hp-above')
  if (!s1HasAbove) return

  pw.targetRules = pw.targetRules.map((step, idx) => {
    if (idx !== 0 && idx !== 1) return step
    if (idx === 0 && typeof step === 'object' && step.rule === 'self' && Array.isArray(step.whenAll)) {
      return {
        ...step,
        whenAll: step.whenAll.map((w) =>
          w?.when === 'self-hp-below' ? { when: 'self-hp-above', value: w.value } : w,
        ),
      }
    }
    if (idx === 1 && typeof step === 'object' && step.rule === 'lowest-hp-ally' && Array.isArray(step.whenAll)) {
      return {
        ...step,
        whenAll: step.whenAll.filter((w) => w?.when !== 'self-no-shield'),
      }
    }
    return step
  })
  ...
  warnings.push(
    '已修正：真言术：盾「全队偏高血线时套盾」误写为「自身血量低于」；已改为「自身血量高于」门控。已移除第二段中多余的「自身无盾」（第一段不满足时通常表示自身已有盾，再给无盾队友）。',
  )
}

/**
 * Strip redundant self-hp-above gate from PWS step 2 when step 1 already gates on it.
 * Step 1 failing due to self-no-shield implies self-hp-above passed; flash-heal handles the low-HP case.
 * @param {Object[]} conditions
 * @param {string[]} warnings
 */
function stripRedundantPwsStep2Gates(conditions, warnings) {
  const pw = conditions.find((c) => c.skillId === 'power-word-shield')
  if (!pw?.targetRules || pw.targetRules.length < 2) return

  const s0 = pw.targetRules[0]
  if (typeof s0 !== 'object' || s0 === null) return
  const s0wa = s0.whenAll
  if (!Array.isArray(s0wa)) return
  const s0HasHpAbove = s0wa.some((w) => w?.when === 'self-hp-above')
  if (!s0HasHpAbove) return

  let changed = false
  for (let i = 1; i < pw.targetRules.length; i++) {
    const step = pw.targetRules[i]
    if (typeof step !== 'object' || step === null) continue
    if (step.rule !== 'lowest-hp-ally') continue
    const wa = step.whenAll
    if (Array.isArray(wa)) {
      const filtered = wa.filter((w) => w?.when !== 'self-hp-above')
      if (filtered.length < wa.length) {
        changed = true
        if (filtered.length === 0) {
          pw.targetRules[i] = step.rule
        } else if (filtered.length === 1) {
          delete step.whenAll
          step.when = filtered[0].when
          if (filtered[0].value !== undefined) step.value = filtered[0].value
        } else {
          step.whenAll = filtered
        }
      }
    }
    if (step.when === 'self-hp-above') {
      changed = true
      pw.targetRules[i] = step.rule
    }
  }
  if (changed) {
    warnings.push(
      '已修正：真言术：盾第二段「血量最低的队友」不需要再检查「自身血量高于」门控（第一段已包含该检查；失败时快速治疗优先处理低血）。',
    )
  }
}
...
export function validateAiTactics(raw, skillIds, heroClass, userInput) {
  ...
  const conditions = (raw.conditions || [])
    ...
    .map((c) => {
      ...
      if (c.when && VALID_WHEN.has(c.when)) {
        whenClauses.push({ when: c.when, value: c.value })
      }
      if (Array.isArray(c.whenAll)) {
        for (const w of c.whenAll) {
          if (w && VALID_WHEN.has(w.when)) {
            whenClauses.push({ when: w.when, value: w.value })
          }
        }
      }
      if (whenClauses.length > 0) {
        ...
        const seen = new Set()
        for (const w of whenClauses) {
          if (isNonsensicalCondition(w.when, w.value)) continue
          const key = `${w.when}:${w.value ?? ''}`
          if (seen.has(key)) continue
          seen.add(key)
          deduped.push(w)
        ...
        if (deduped.length > 0) {
          if (stripWhen) {
            warnings.push(
              `已移除 AI 编造的条件「${c.skillId}」（玩家未提及触发条件；含多段 when / whenAll 时整项移除）`,
            )
          } else if (deduped.length === 1) {
            clean.when = deduped[0].when
            if (deduped[0].value !== undefined) clean.value = deduped[0].value
          } else {
            clean.whenAll = deduped
          }
        } else {
          for (const w of whenClauses) {
            if (isNonsensicalCondition(w.when, w.value)) {
              warnings.push(`已移除无意义条件「${c.skillId}：${w.when} ${w.value ?? ''}」`)
            }
          }
        }
      }
  ...
  if (heroClass === 'Mage') {
    ...
    if (mageBand?.targetRule) {
      const vr = new Set([...VALID_TARGET_RULES_ENEMY])
      if (vr.has(mageBand.targetRule)) {
        targetRule = mageBand.targetRule
      }
    }
  }
...
export function targetRuleStepDisplay(step, opts = {}) {
  const ruleStr = typeof step === 'string' ? step : step?.rule
  const isPwsAlly =
    opts.skillId === 'power-word-shield' && ruleStr === 'lowest-hp-ally'
  const baseLabel = isPwsAlly ? '血量最低的无盾队友' : targetRuleDisplayName(ruleStr)

  if (typeof step === 'string') return baseLabel
  if (typeof step === 'object' && step !== null && typeof step.rule === 'string') {
    ...
    if (step.when && WHEN_DISPLAY[step.when]) {
      ...
      return valuePart ? `${baseLabel}（${whenPart} ${valuePart}）` : `${baseLabel}（${whenPart}）`
    }
    return baseLabel
  }
  return String(step)
 ...
 * @returns {string}
 */
export function targetRulesChainDisplay(steps, opts = {}) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    out.push(targetRuleStepDisplay(steps[i], opts))
  }
  return out.join('')
}
...
/**
 * Format skill-level tactics gates (single when or whenAll AND) for UI preview.
 * @param {{ when?: string, value?: *, whenAll?: { when: string, value?: * }[] }} cond
 * @returns {string}
 */
export function tacticsSkillWhenDisplay(cond) {
  if (!cond) return ''
  if (Array.isArray(cond.whenAll) && cond.whenAll.length > 0) {
    return cond.whenAll
      .map((w) => {
        if (!w || !w.when) return ''
        const label = whenDisplayName(w.when)
        const val = w.value !== undefined ? ` ${conditionValueDisplay(w.when, w.value)}` : ''
        return `${label}${val}`
      })
      .filter(Boolean)
      .join('；')
  ...
  if (cond.when) {
    const label = whenDisplayName(cond.when)
    const val = cond.value !== undefined ? ` ${conditionValueDisplay(cond.when, cond.value)}` : ''
    return `${label}${val}`
  }
  return ''
}
