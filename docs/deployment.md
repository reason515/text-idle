...
- **Restart**: scheduler resumes WallClock due users from `next_tick_at`; stuck client-gated rows migrate to WallClock on startup backfill; earnings capped by `offline_cap_until` (see [design/15-server-combat-tick.md](./design/15-server-combat-tick.md)).
