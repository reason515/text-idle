...
- **Recruitment flow order**: (1) Select hero from roster (same as Example 4); (2) Allocate attribute points (**12 for Lv 5**, **27 for Lv 10** — `3 × (level - 1)` per [04-classes-attributes.md](design/04-classes-attributes.md) 1.6); (3) Select initial skill (Warrior: 3 options per Example 12; Mage: Frostbolt / Fireball pick 1); (4) Complete **all** unresolved skill milestones up to join level (e.g., Lv 5 recruit: milestone 3 only; Lv 10 recruit: 3, 6, 9, 10 in order, per Example 26).
...
| AC2  | Player triggers squad expansion after first map boss defeat                                                | Player enters the recruitment flow    | The new hero is created at **level 5** with base attributes from their class; **12** unassigned attribute points are available (3 per level from 1 to 5)         |
...
| AC8  | Player has defeated the zone boss on the second map (Westfall)                                             | Boss is defeated                      | The third map (Duskwood) is unlocked; recruit modal may appear if a seat is open; squad expansion available for the 5th hero (level 10) when `squad.length < 5`                                        |
