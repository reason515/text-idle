...
| #   | Given                                                                                    | When                                                       | Then                                                                                                                                                                                                                       |
| --- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1 | Player is on the hero preview step (after intro and team name, per Example 3)            | Player clicks "Start Adventure"                            | The adventure begins with a fixed trio (Warrior, Mage, Priest); no character selection for initial squad; player is redirected to the main screen                                                                          |
...
- **Recruit prompt after boss (map 1–2 only)**: When a map 1 or 2 zone boss is defeated and the squad has an open seat, a **Recruit squad expansion** modal appears after the victory summary ( **Recruit now** or **Later** ). Later closes the modal and auto-combat continues; the **+ Recruit** button remains available. Map 3–5 boss victories unlock the next map only—no recruit modal and no "squad full" message.
...
| AC4 | Player has defeated the zone boss on the current map                                 | Boss is defeated                                                                 | The next map is unlocked; player can select the new map to explore via the map modal; if the boss was on **map 1 or 2** and the squad has an open seat, a **Recruit squad expansion** modal appears (Recruit now / Later); squad expansion is also available via **+ Recruit** (Example 4 AC4, Example 27) |
| AC4b | Player has defeated the zone boss on **map 3, 4, or 5**                             | Boss is defeated                                                                 | The next map is unlocked; **no** recruit modal and **no** "squad full" UI message; auto-combat continues after the victory summary                                                                                                      |
| AC4c | Player sees the recruit modal after a map 1 or 2 boss victory                       | Player clicks **Later**                                                          | The modal closes; auto-combat continues; **+ Recruit** remains visible while `squad.length < recruitLimit`                                                                                                                                 |
...
| AC5 | Squad won the last combat with 1 hero having died (e.g., revived during combat or died and was rezzed) | Rest phase begins                                                       | Recovery takes longer than if no hero had died; after full recovery **5 extra rest steps** are required per death; penalty scales with death count |
| AC6 | Squad won the last combat with 2 heroes having died                                                    | Rest phase begins                                                       | Recovery takes even longer than with 1 death; **10 extra rest steps** after full recovery (2 × 5); extra steps count toward exploration-step efficiency |
...
| AC8  | Squad enters combat on a specific map (e.g., Elwynn Forest or Westfall)               | Encounter is generated         | Monsters are drawn from **that map's pool only** (e.g., Elwynn: Young Wolf, Kobold Miner, …, Hogger; Westfall: Defias Bandit, Harvest Watcher, …, VanCleef — not Elwynn wolves) |
...
| AC31 | A hero blocks a physical hit from a monster                                                                 | Player views the combat log detail for that hit     | The log includes a block line (e.g. 格挡成功); if block damage reduction applied, a line with post-block effective damage; if block counter dealt damage, a line with the counter amount and attacker name |
...
- **Learn milestone**: Levels **10, 20, 30, 40, 50, 60** — player may learn one of **3 fixed new skills** mapped from the legacy tier table (e.g., Lv 10 learn pool = former Lv 5 row: Cleave, Whirlwind, Defensive Stance for Warrior; for Priest: Greater Heal, Fade Mind, Shadow Word: Pain; for **Druid**: Bear Form, Regrowth, Rake).
- **Overlap**: Levels **30** and **60** are both multiples of 3 and 10 — the same inline panel may show both "Enhance existing" and "Learn new" sections; the player still picks one action per milestone as implemented.
- **Levels that are only learn (10, 20, 40, 50)**: Not multiples of 3 — **only** "Learn new" is offered (no enhance on that level-up).
- **Choice window**: Battle log hints when a milestone is reached; inline skill choice panel on hero detail **技能** tab (no modal on main screen); player may defer; reopen via **技能** tab for the **lowest** unresolved milestone; red-dot badges on hero card and tabs when attribute or skill points are pending.
- **Skill detail**: Each skill card shows current display level (Lv.1–5) and a **强化成长** ladder listing each upgrade tier effect.
...
| #    | Given                                                                                    | When                                                                                | Then                                                                                                                                                                                                                |
| ---- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1  | A hero (e.g., Warrior) gains enough XP to level from 2 to 3                              | Level-up is triggered                                                               | Battle log shows skill milestone hint for **Lv 3**; hero card shows red dot; inline panel on **技能** tab offers enhance or defer; **Learn new** is not shown (no learn pool at 3) |
| AC2  | Player opens hero detail **技能** tab with unresolved Lv 3 milestone (Warrior)           | Player views the inline panel                                                       | "Enhance existing skill" lists current skills; no "Learn new" row for tier skills                                                                                                       |
| AC3  | Player chooses "Enhance existing skill" at a valid milestone                             | Player confirms                                                                     | One skill's `enhanceCount` increases (max 4); panel updates or clears for next milestone                                                                                                |
| AC4  | Player levels from 9 to 10 (Warrior)                                                     | Level-up is triggered                                                               | Battle log hint for **Lv 10**; **Learn new** shows Cleave, Whirlwind, Defensive Stance (first learn pool); enhance section may be absent (10 is not a multiple of 3)                    |
| AC5  | Player chooses "Learn new skill" at Lv 10 and picks Cleave                               | Player confirms                                                                     | Cleave is added to the hero's skills                                                                                                                                                    |
| AC5a | Priest levels from 9 to 10                                                               | Level-up is triggered                                                               | Inline panel for **Lv 10** shows Priest learn options: Greater Heal (Holy), Fade Mind (Discipline), Shadow Word: Pain (Shadow)                                                          |
| AC5c | Druid levels from 9 to 10 (or resolves Lv 10 milestone during 4th-hero recruitment)      | Level-up or onboarding milestone is triggered                                       | Inline panel for **Lv 10** shows Druid learn options: Bear Form (Guardian), Regrowth (Restoration), Rake (Feral, **single target**)                                                     |
| AC6  | A hero has already learned all 3 skills from the current learn pool at a learn milestone | The hero reaches that learn milestone again (not applicable twice) or pool is empty | Only enhance is available at 3-multiple milestones; at learn milestones with all pool skills learned, only enhance appears if the level is also a multiple of 3 (e.g. 30, 60), else panel may offer no valid choice |
| AC7  | Player defers skill choice (does not confirm)                                              | Player closes detail or continues combat                                            | No change; milestone remains unresolved; red dot persists                                                                                                                               |
| AC8  | Multiple heroes level up with skill milestones                                           | Level-up is triggered                                                               | Battle log shows hints per hero/level; no blocking modal                                                                                                                                |
| AC11 | Player has unresolved skill milestone                                                      | Player opens hero detail **技能** tab                                                 | Inline panel shows the lowest unresolved milestone (e.g. 3, then 6, then 10)                                                                                                             |
...
- **Trigger (4th hero)**: Player defeats the zone boss on the first map (e.g., Hogger in Elwynn Forest). Squad expands from 3 to 4. **Druid only** (Malfurion).
...
| #    | Given                                                                                                      | When                                  | Then                                                                                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1  | Player has defeated the zone boss on the first map (Elwynn Forest)                                         | Boss is defeated                      | The second map (Westfall) is unlocked; a **Recruit squad expansion** modal appears; player can choose **Recruit now** or **Later**; **Druid-only** 4th seat (no hero-select grid)          |
...
- **Configuration UI**: Natural-language input + AI parse (SiliconFlow Qwen3-8B) + Apply merges into saved tactics; read-only **current tactics** summary (skill IDs such as `basic-attack` are shown with localized names, e.g. 普通攻击). **Template example**: Tactics tab auto-fills a large shared **natural-language editor** (same box for template and edits) when empty; **恢复模板** restores template text. **转为自然语言** (in the current-tactics section) loads only the hero's saved tactics into the input via `getCurrentTacticsNaturalLanguage` for fine-tuning. Normal-attack targeting uses `conditions` entry `skillId: basic-attack` (same target-rule vocabulary as skills). **`basic-attack` may also appear in `skillPriority`** when the player wants normal attack attempted before other skills (that turn **does not** use implicit end-of-priority fallback basic attack, whether or not the explicit slot fired). See [10-tactics.md](design/10-tactics.md) section 7.
- **Skill priority**: Ordered list of skill IDs (plus optional `basic-attack`); the first entry that passes resource, cooldown, and conditions is used; if none **and** `skillPriority` omits `basic-attack`, basic attack via implicit fallback. **Mage/Priest**: when **no** spell in priority is affordable for lack of MP **and** the turn reaches implicit fallback basic attack, that fallback **does not** apply `conditions` for `skillId: basic-attack` (HP-ratio gates relaxed per design); explicit `basic-attack` in `skillPriority` uses full gates and **never** triggers that relax path. **Warrior** keeps full `basic-attack` conditions when skills fail for lack of rage. See [10-tactics.md](design/10-tactics.md) section 4.
...
- **targetRules chain**: Per-skill array tried in order until a target is found; if none, skill is skipped. Step-level `target-hp-below` / `target-hp-above` / `target-has-debuff` gates run **after** the rule picks a candidate (not as a pre-pick party gate). See [10-tactics.md](design/10-tactics.md).
...
- **Apply merge**: Incoming non-empty `skillPriority` replaces stored order. Incoming always includes `targetRule` key from validation (`null` removes stored global default so stale values do not persist). `conditions` merge by `skillId`. Solo-survivor supplements merge `allies-alive-gte` onto heal/shield without replacing `targetRules`, and prepend `solo-survivor` onto `basic-attack` chains (see [10-tactics.md](design/10-tactics.md) section 7).
- **AI validate supplement**: If user text mentions rage/mana shortage then normal attack (or 否则普攻) but parsed JSON omits `basic-attack` in `conditions`, frontend validation may append `basic-attack` target rules and show a warning (see `frontend/src/game/aiTactics.js`). If user text mentions solo survivor (只剩自己 / 独自存活 / 队友全灭), validation adds `solo-survivor` basic-attack step and `allies-alive-gte` gates on heals/shields, and drops mistaken `self-if-enemy-targeting` rewrites. **Priest party triage** (solo attack, enemy execute band, ally heal %, healthy-party shield on unshielded allies) is supplemented in `validateAiTactics` when natural language mentions heal + shield + basic attack with both ally and enemy HP thresholds (see `e2e/browser/tactics.spec.js` Example 28 AC9).
...
| #    | Given                                                                                       | When                                                                      | Then                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1  | Player is on the main screen or squad view                                                  | Player opens a hero and the Tactics tab                                   | The AI tactics section is shown (natural language input and AI parse action); a **template example** card reflects current/default tactics; the input is pre-filled from the template when empty; player can edit text and parse                                                                                                                                                                                                                                                                                                     |
| AC2  | Player has selected a hero and opened the Tactics tab                                       | Player views the panel                                                    | A read-only **current tactics** area shows applied priority, default target, and per-skill rules when configured; **转为自然语言** is enabled when tactics exist and fills the input with the saved config as formatted natural language; or a hint when nothing is configured                                                                                                                                                                                                                                                              |
...
| AC9  | Priest party triage text is parsed (E2E may mock AI JSON) and applied                         | Player opens priest Tactics tab, runs AI parse, applies preview           | Current tactics summary shows **60%** ally heal gate, **10%** enemy execute postpone on heals/shields, **unshielded lowest ally** for PW:S (not tank-only), and **solo-survivor** basic attack; persisted save matches                                                                                                                                                                                            |
| AC10 | Hero has condition `{ skillId: 'shield-slam', when: 'target-has-debuff', value: 'sunder' }` | Warrior's turn and no enemy has Sunder debuff                             | Shield Slam is skipped (no valid target); next skill in priority (e.g., Sunder Armor) is tried                                                                                                                                                                                                                                                                                                                      |
| AC11 | Hero has condition `{ skillId: 'taunt', when: 'ally-ot' }`                                  | Combat state where ally-ot is false                                       | Taunt is skipped when the condition is not met; next skill in priority is tried                                                                                                                                                                                                                                                                                                                                     |
| AC12 | Hero has global target rule `lowest-hp` and performs basic attack                           | No skill in priority is usable                                            | Basic attack uses the same target-rule resolution as other enemy-targeting actions for that hero                                                                                                                                                                                                                                                                                                                    |
| AC13 | Player applies a second AI parse that only adds per-skill `conditions`                      | Player clicks Apply                                                       | Existing `skillPriority` and `targetRule` remain unless the new result explicitly includes them; conditions merge by `skillId`                                                                                                                                                                                                                                                                                      |
| AC14 | Priest has heal/shield tactics; player adds solo-survivor rule via AI parse and Apply       | Only the priest is alive in combat; enemies are above execute band        | Priest attempts **basic attack** on lowest-HP enemy; **flash-heal** and **power-word-shield** are skipped due to `allies-alive-gte` (>= 2 alive allies required); existing heal/shield target chains remain when party has teammates                                                                                                                                                                                |
| AC15 | Warrior text mentions rage shortage then normal attack (E2E may mock AI JSON)               | Player opens warrior Tactics tab, runs AI parse, applies preview          | Preview/current tactics include **basic-attack** target chain (`default` -> `lowest-hp` when sunder uses tank fallback); warning about supplement when AI omitted basic attack                                                                                                                                                                                                                                        |
| AC16 | Mage numbered HP-band text (5% / 5-50% / 50%+) is parsed (E2E may mock AI JSON)             | Player opens mage Tactics tab, runs AI parse, applies preview             | Preview/current tactics show **frostbolt** above 50%, **fireball** between 5% and 50%, **basic attack** below 5% on lowest-HP target; persisted save matches                                                                                                                                                                                                                                                      |
...
- **Priest flash-heal**: `checkPriestFlashHealSkillAllowed` prevents casting on a full party when the only chain step is emergency `ally-hp-below` with no skill-level `when` (see [10-tactics.md](design/10-tactics.md) section 4, Flash Heal note). When the first step adds **`whenAll`** (e.g. **`enemy-all-hp-above`**), emergency ally triage must still pass **full step gates** via `evaluateTargetRuleStepGates` before the skill is allowed.
...
| AC9 | Priest flash-heal first chain step uses `whenAll` with `ally-hp-below` and `enemy-all-hp-above`; an ally is below the heal threshold but some enemy HP ratio is **≤** the execute cutoff | Priest's turn                                    | Flash Heal is **not** cast; higher-priority spells continue or basic attack may resolve                                                                                                                     |
