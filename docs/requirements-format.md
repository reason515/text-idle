...
- **Conditions**: Per-skill triggers (target-hp-below, target-has-debuff, ally-ot, etc.); `target-has-debuff` filters the target pool first. For **target-hp-below** and **target-hp-above**, the engine does **not** pre-filter by HP ratio: it applies `targetRule` on **all** alive enemies, then evaluates the skill-level `when` on that pick. For **target-has-debuff**, the engine picks a target from the filtered pool, then evaluates. Self/ally/round-only conditions are evaluated before target pick where applicable. See [10-tactics.md](design/10-tactics.md) section 4.
