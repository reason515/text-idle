...
- **Configuration UI**: Natural-language input + AI parse (SiliconFlow Qwen3-8B) + Apply merges into saved tactics; read-only **current tactics** summary (skill IDs such as `basic-attack` are shown with localized names, e.g. 普通攻击). Normal-attack targeting uses `conditions` entry `skillId: basic-attack` (same target-rule vocabulary as skills). **`basic-attack` may also appear in `skillPriority`** when the player wants normal attack attempted before other skills (engine skips implicit fallback duplicate swing that turn). See [10-tactics.md](design/10-tactics.md) section 7.
- **Skill priority**: Ordered list of skill IDs (plus optional `basic-attack`); the first entry that passes resource, cooldown, and conditions is used; if none, basic attack via implicit fallback. **Mage/Priest**: when **no** spell in priority is affordable for lack of MP **and** the turn reaches implicit fallback basic attack, that fallback **does not** apply `conditions` for `skillId: basic-attack` (HP-ratio gates relaxed per design); explicit `basic-attack` slots in priority still use full `basic-attack` conditions. **Warrior** keeps full `basic-attack` conditions when skills fail for lack of rage. See [10-tactics.md](design/10-tactics.md) section 4.
