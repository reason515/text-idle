...
- **Apply merge**: Incoming non-empty `skillPriority` replaces stored order. Incoming always includes `targetRule` key from validation (`null` removes stored global default so stale values do not persist). `conditions` merge by `skillId`.
