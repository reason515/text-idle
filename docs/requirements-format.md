...
- **Squad expansion**: After defeating map 1 boss (Hogger) → recruit **4th** hero (any available class except fixed trio duplicates); after defeating map 2 boss (VanCleef) → recruit **5th** hero (**Druid only**, Malfurion). Max 5 heroes.
- **Classes (4th hero)**: Warrior, Paladin, Priest, Mage, Rogue, Hunter, Warlock, Shaman (and Druid only if not yet in squad — but **5th slot is Druid-only**).
...
| AC4 | Player has fewer than 5 characters and has defeated map 1 boss | Player triggers squad expansion for the **4th** seat | Player can select any available non-trio hero; join level **5**; Warrior/Mage initial skill pick applies (Example 27) |
| AC4b | Player has 4 characters and has defeated map 2 boss | Player triggers squad expansion for the **5th** seat | Only **Druid** (Malfurion) is available; join level equals **minimum level in current squad**; **2 fixed skills** (Rejuvenation, Maul); no skill pick step |
...
| AC5c | Druid levels from 9 to 10 (or resolves Lv 10 milestone during 5th-hero recruitment)      | Level-up or onboarding milestone is triggered                                       | Modal for **Lv 10** shows Druid learn options: Bear Form (Guardian), Regrowth (Restoration), Rake (Feral, **single target**)                                                                                        |
...
- **Trigger (5th hero)**: Player defeats the zone boss on the second map (e.g., VanCleef in Westfall). Squad expands from 4 to 5. **Druid only** (Malfurion). Max squad size reached.
- **Expansion hero level**: 4th hero joins at **level 5**. 5th hero (Druid) joins at **min level in current squad** (e.g., squad min Lv7 → Druid Lv7 with `3 × 6 = 18` attribute points).
- **Recruitment flow (4th hero)**: (1) Select hero; (2) Allocate **catch-up** attribute points (`3 × (level - 1)`); (3) Initial skill if Warrior/Mage; (4) **Optional** skill milestones up to join level (skippable).
- **Recruitment flow (5th hero, Druid)**: (1) Confirm Malfurion (only option); (2) Allocate catch-up attribute points; (3) **No** skill pick — fixed **Rejuvenation + Maul**; (4) **Optional** skill milestones (skippable). Initial skill enhance uses scheme A (see [05-skills.md](design/05-skills.md) 8.4.4); Lv 10 learn pool: Bear Form / Regrowth / Rake (8.4.5); Lv 20+ learn pools TBD.
...
| #    | Given                                                                                                      | When                                  | Then                                                                                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1  | Player has defeated the zone boss on the first map (Elwynn Forest)                                         | Boss is defeated                      | The second map (Westfall) is unlocked; a **Recruit squad expansion** modal appears; player can choose **Recruit now** or **Later**; player can select a new hero to join as the 4th squad member          |
...
| AC2  | Player triggers squad expansion after first map boss defeat                                                | Player enters the recruitment flow    | The new hero joins at **level 5**; **12 catch-up attribute points** must be allocated in onboarding (`3 × (4 level-ups)`); `unassignedPoints` starts at **0** until the next level-up |
...
| AC4  | Player has completed attribute allocation (4th hero, Warrior) | Player proceeds | Initial skill selection is shown for Warrior/Mage; other classes skip to confirm or milestones |
| AC4d | Player triggers 5th-hero (Druid) recruitment after map 2 boss | Player enters recruitment | Only Malfurion (Druid) is listed; subtitle states fixed skills and min-squad-level join |
| AC5  | Player has selected the initial skill (Warrior/Mage, 4th hero) | Player proceeds | **Optional** skill milestone UI may appear (Example 26); player may enhance/learn or **skip** (skip does not consume the milestone) |
| AC5b | Player completes Druid attribute allocation (5th hero) | Player proceeds | Confirm step shows **fixed** Rejuvenation and Maul; no skill pick step |
...
| AC7  | Player views the squad after recruiting | Squad panel | 4th hero shows Lv5 (typical); 5th Druid shows **squad min level** and 2 fixed skills |
| AC7b | An expansion hero (e.g., Lv5 Warrior) gains XP and levels up after joining | Level-up occurs | Hero receives **+3 unassignedPoints**; player can allocate them in hero detail like the fixed trio |
...
| AC8  | Player has defeated the zone boss on the second map (Westfall) | Boss is defeated | Third map unlocked; recruit modal for **Druid-only** 5th seat when `squad.length === 4` |
