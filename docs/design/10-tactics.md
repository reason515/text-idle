...
| threat-not-tank-lowest-hp | 在「仇恨表第一名**不是**指定坦克」的存活怪中，**当前 HP 最低**的一只（需指定坦克）；筛子集为空时的行为与 `threat-not-tank-random` 相同 |
...
**`threat-not-tank-random`（非坦克第一仇恨）**：筛子集为空时行为分两种：（1）**全场威胁仍为 0**（战斗尚未产生威胁，开局）：在全部存活敌方候选中随机一只，以便第一回合仍能选中目标；（2）**已存在非零威胁**且每只怪的第一仇恨均为指定坦克：返回 **null**（无目标），该步在目标链中失败，技能可被跳过或由下一规则步承接。第一仇恨的判定与怪物实际选目标一致（含仇恨并列时「上一击粘性」tie-break，见 [12-threat.md](./12-threat.md) 2.2）。若玩家希望在「全员在打坦克」时仍对怪施法（如破甲），应为该技能配置 `targetRules: ['threat-not-tank-random', 'lowest-hp']` 等链；**嘲讽**通常应**仅**使用单步 `threat-not-tank-random`，避免在无可拉怪对象时误嘲血量最低的怪。
...
**主界面战术页**：选择英雄 → **AI 战术配置**（自然语言）→ **AI 解析** → **解析预览** → **应用** / **放弃**；下方为 **当前战术** 只读摘要（出手顺序 / 技能优先级、默认目标、单技能规则），可 **转为自然语言**（将已保存战术填入上方输入框以便微调）或 **清空全部战术**。**模板示例**：战术页展示由当前存档战术（若无则用固定三人组默认战术）转换的**场景化自然语言**（编号句式，按 OT/血线/治疗等情境描述）；首次进入战术页且编辑框为空时自动填入；**恢复模板** 可重置编辑框内容。**转为自然语言** 将已保存战术填入同一编辑框。编辑框与模板共用，可直接修改全文。点击 **AI 解析** 后，在 API 返回前显示 **处理中** 等待卡片（旋转指示 + 说明文案），输入框与提交按钮禁用，避免重复提交。解析预览中的 **技能优先级** 若未显式包含 **普通攻击**，会在列表末尾追加 **普通攻击**（与引擎「优先级遍历结束后隐式普攻」一致）；若模型已将 **`basic-attack`** 写入 **`skillPriority`**（例如斩杀优先），则按结构化顺序展示、通常不再重复追加末尾普攻说明。**牧师斩杀让路**（快速治疗 / 真言术：盾含 **`enemy-all-hp-above`** 且 **`skillPriority` 首位不是 `basic-attack`**）：UI 将行标题改为 **出手顺序**，并按玩家语义展示 **普通攻击 → 快速治疗 → 真言术：盾**（引擎仍先遍历法术、由门控跳过后再隐式普攻；展示顺序仅为可读性）。此时 **不再** 在链尾重复追加普攻。预览与 **当前战术** 均追加 **说明** 一行（含斩杀线百分比、缺蓝兜底）。若 **`skillPriority` 首位为 `basic-attack`**，标题仍为 **技能优先级**，说明侧重「先普攻再法术」。

**自然语言 → 结构化配置**：前端调用 SiliconFlow（OpenAI 兼容接口）的 **Qwen3-8B** 模型，将玩家输入解析为 `skillPriority`、`targetRule`、`conditions`（与第二节数据模型一致）。实现见 `frontend/src/game/aiTactics.js`（系统提示词、输出校验、合并逻辑）。客户端对模型原文做 **JSON 容错**（请求侧启用 **JSON object** 输出模式；解析侧去掉代码围栏、提取首个平衡花括号对象、去除尾随逗号，必要时用 **jsonrepair** 修复后再 `JSON.parse`），减少偶发解析失败。**校验补充**：若玩家原文同时出现 **怒气不足/法力不足…普通攻击（或普攻）** 或 **否则普通攻击**，而模型未输出 `conditions` 中的 **basic-attack**，`validateAiTactics` 会 **自动追加** `{ skillId: 'basic-attack', targetRules: ['default', 'lowest-hp'] }` 并给出「已补充」类警告，避免解析预览缺少普攻目标链。若玩家描述 **非坦克怪 → 嘲讽 → 破甲 → 普攻**、**全员打坦克时破甲打最低血且不嘲讽** 等坦克拉怪模板（含「目标非坦克」「其中 HP 最低」等措辞），校验会 **补全** 标准 `skillPriority`（嘲讽在破甲前）与 per-skill 目标链；当原文强调 **非坦克怪中 HP 最低** 时使用 **`threat-not-tank-lowest-hp`**（而非全局 `lowest-hp` 或随机 OT）。若原文同时出现 **无敌人以自己为目标 / 没有目标为自己的敌人**、**坦克血量低于 70%**、**快速治疗（或治疗坦克）**，而 **flash-heal** 未含任何 `tank-hp-below` 门控，会 **自动插入** 目标链一步：`{ rule: 'tank', whenAll: [ tank-hp-below 0.7, enemy-not-targeting-self ] }`（插在紧急 `ally-hp-below` 步之后），并给出「已补充」类警告，避免模型只写盾链、漏写坦克低血量治疗链。
...
固定战法牧在 `createFixedTrioSquad()` 中写入以下战术，使玩家在**未配置战术**时也能稳定作战；熟悉系统后可按个人偏好用 AI 战术页覆盖。

| 职业 | 意图 | 要点 |
|------|------|------|
| 战士 | 拉怪 + 破甲 + 怒气不足普攻 | 嘲讽优先对「非坦克第一仇恨」中 **HP 最低** 的怪（`threat-not-tank-lowest-hp`）；全员打坦克时嘲讽无目标则跳过；破甲对 OT 怪或全场最低血；含 `basic-attack` 怒气兜底 |
| 法师 | 按目标血量分段输出 | 全场 **HP 最低** 为目标：>50% 寒冰箭、5%–50% 火球（`whenAll` 上下阈）、≤5% 普攻；缺蓝时隐式普攻仍按 `lowest-hp` |
| 牧师 | 急诊治疗优先、安全时套盾、闲时普攻 | 快速治疗：队友 ≤70% 急诊；被盯上且自身 <60% 自疗；坦克 <70% 且未被盯上时奶坦；真言术：盾：被盯上且无盾则盾自己；坦克 ≥70% 且无盾则盾坦；法力不足时普攻最低血敌人 |

常量与结构见 `frontend/src/data/heroes.js`（`FIXED_TRIO_*_TACTICS`）。
