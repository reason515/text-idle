...
|----------|------|------------|----------|--------------|--------------|
| 击败第 1 张地图 BOSS（如 Hogger） | 第 4 人 | **除固定三人外任意可选英雄**（战法牧已在队则不可重复） | **5 级** | 12 点（1→5 共 4 次升级 × 3 点） | 1. 选择英雄 2. 分配属性点 3. 选择初始技能（仅战士 3 选 1、法师 2 选 1；其他职业无此步）4. **可选**：处理未决技能里程碑（可跳过） |
| 击败第 2 张地图 BOSS（如 VanCleef） | 第 5 人 | **限定德鲁伊**（玛法里奥·怒风） | **与当前小队等级最低的英雄相同** | `3 × (加入等级 - 1)` | 1. 确认德鲁伊（无英雄选择）2. 分配属性点 3. **固定 2 个初始技能**（回春术、重殴，无需选择）4. **可选**：处理未决技能里程碑（可跳过，见下） |

- **设计意图**：第一次扩充仍让玩家自由选职业与（战/法）初始技能；第二次扩充引入德鲁伊作为全能补位，等级跟随小队进度，避免固定 Lv10 与主力等级脱节。
- **德鲁伊加入等级**：取招募时刻小队中**最低**英雄等级（含固定三人与其他已招募扩展英雄）；例：小队为 Lv7 / Lv8 / Lv9 / Lv10 时，德鲁伊以 **Lv7** 加入，待分配 `3 × 6 = 18` 点。
- **流程顺序**：英雄选择 → 属性点分配（**必须全部分配**）→ 初始技能选择（仅第 4 人且战士/法师）→ 技能里程碑（**可选，可跳过**）→ 确认加入。
...
| **表现** | 胜利结算日志揭示后，弹出**扩充小队** Modal；第 4 人席位说明 Lv.5 与可选英雄，第 5 人席位说明**限定德鲁伊**与**小队最低等级** |
