...
- **默认战术**：三人组自带预配置 `tactics`（无需玩家先调战术即可开局作战），规则见 [10-tactics.md](./10-tactics.md)「固定三人组默认战术」；实现见 `createFixedTrioSquad()`。
...
| 击败第 2 张地图 BOSS（如 VanCleef） | 第 5 人 | **除固定三人与已在队德鲁伊外任意可选英雄** | **与当前小队等级最低的英雄相同** | `3 × (加入等级 - 1)` | 1. 选择英雄 2. 分配属性点 3. **固定 2 个初始技能**（该职业 8.x，无需选择）4. **可选**：处理未决技能里程碑（可跳过） |

- **设计意图**：两次扩充的加入等级均跟随小队进度（取最低等级），避免固定 Lv5/Lv10 与主力等级脱节；第 4 人为德鲁伊固定 2 技能；第 5 人任选英雄，**固定 2 初始技能**（与战法牧、德鲁伊同一规则，见 [05-skills.md](./05-skills.md) 三）。
...
- **流程顺序（第 4 人德鲁伊）**：属性点分配（**必须全部分配**）→ 技能里程碑（**可选，可跳过**）→ 确认加入。**流程顺序（第 5 人）**：英雄选择 → 属性点分配 → **无**初始技能选择步 → 技能里程碑（可选）→ 确认加入。
