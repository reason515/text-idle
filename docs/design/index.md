...
| **玩家数据统计** | [13-player-statistics.md](./13-player-statistics.md) | 唯一步数口径、探索步数、展示倍率、战斗与休息**日志步间毫秒一致**（7.5）、详情 Modal Tab（概览 / 场次趋势 / **伤害统计**）、按技能拆分、实际暴击/命中/闪避、图表与表 |
| **音效（表现层）** | [14-audio.md](./14-audio.md) | 样本优先（Kenney CC0）+ Web Audio 合成回退；按事件区分（物理/魔法/混合/闪避/DoT/阵亡/胜负）；E2E 快速模式与后台标签页静音 |
