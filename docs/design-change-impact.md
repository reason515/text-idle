...
| Check | Affected areas | Notes |
|-------|----------------|-------|
| **New metrics or event sources** | [13-player-statistics.md](design/13-player-statistics.md) | 唯一步数分母、无按回合**比率**、战斗行动步与休息步、探索步数、展示倍率、清零、每场战斗/休息后更新；**概览 Tab**：`battleCount` / `victoryCount`、胜率饼图；**场次趋势 Tab**：每场回合数/本场金币/本场经验序列（归一化展示）、图内图例与悬停数值提示、`battleTimeline` 上限与清零；**伤害累计**：`damageByHero` 含 `skillById`（按技能 ID）与各角色构成饼图、悬停显示数值与占比；**受伤累计**：`injuryByHero`（含护盾吸收、敌方普攻/技能构成饼图，见 13 文档 7.6） |
